...
Decant is local-first, no-inference-cost (your own AI), citation-grounded, and open-source (shell).
Near-term, in rough priority:

- **Split** — segment a packet into labeled sections (e.g. a statement bundling several accounts).
- **Local MCP server** — expose `extract / parse / chunk` as tools so agents in Claude Code / Cursor can call Decant directly (private, no cost).
...
Deliberately out of scope (would break the local-first, no-cost, private model): credit metering, cloud webhooks, a hosted Studio. See [docs/USE-CASES.md](docs/USE-CASES.md) for how Decant differs from cloud APIs.
